...
plt.gca().set_axisbelow(True)  # ensure grid is drawn beneath markers
```
...
*Hint: Use the "inferno" colormap, set marker size to 25, and transparency (alpha) to 0.6. Colour points by Nottingham prognostic index.*
...
If `ages_sorted` isn’t sorted, the plot won’t show a proper cumulative distribution. If you want the y-axis to show percentages, you’d need to normalize the ranks.
